...
#color styles
r2b <- c("#0571B0", "#92C5DE", "#FFFFFF","#F7F7F7", "#F4A582", "#CA0020") #red to blue                                                                               
r2g <- c("#D61818", "#FFAE63", "#FFFFBD", "#B5E384")   #red to green
w2b <- c("#045A8D", "#2B8CBE", "#74A9CF", "#BDC9E1", "#F1EEF6","#FFFFFF")   #white to blue
b2w <- c("#FFFFFF", "#F1EEF6", "#BDC9E1", "#74A9CF", "#2B8CBE", "#045A8D")   #white to blue
lg2dg <- c('#f7fcf5','#e5f5e0','#c7e9c0','#a1d99b','#74c476','#41ab5d','#238b45','#006d2c','#00441b') #light green to dark green
lr2dr <- c('#fff5f0','#fee0d2','#fcbba1','#fc9272','#fb6a4a','#ef3b2c','#cb181d','#a50f15','#67000d') #light red to dark red
lb2db <- c('#f7fbff','#deebf7','#c6dbef','#9ecae1','#6baed6','#4292c6','#2171b5','#08519c','#08306b') #light blue to dark blue
db2lb <- rev(lb2db)
...
#vals <- runif(ndays, -100, 100)
  ...
  #stock data:
  casts <- "MSFT"
  start.date <- "2000-00-01"
  end.date <- "2010-00-01"
      
  casts.data <- read.csv('/Users/bell/Downloads/656695_mod_2010_2015.csv', as.is=TRUE)
  
  # Plot as calendar heatmap
  calendarHeat(casts.data$Date, casts.data$TMINf, varname="TMINf")
}